...
class UnderstandingTurn:
    ...
    action_offerings: dict = field(default_factory=dict)
...
def record_scope_action(message, scope):
    ...
    accepted = (scope.kind == "CONTEXT_REPLY" and not scope.containsUnrelatedTopic
                ...
                # BC-003/007: concrete edits must reach extraction, even when the
                # classifier contradicts itself by labeling them a generic CHANGE.
                and not (scope.replyAction == "CHANGE" and scope.hasRequestDetails))
    if accepted:
        state.actions[message.text] = scope.replyAction
        if scope.offeringCode and scope.confidence >= 0.9:
            state.action_offerings[message.text] = scope.offeringCode
...
def semantic_action_offering(text):
    state = _turn.get()
    return state.action_offerings.get(text) if state and state.enabled else None
...
def apply_order_extraction(extraction, text, existing, require_full=False, *, allow_partial=False):
    ...
    for edit in extraction.edits:
        ...
        if edit.quantity is not None and not same_quantity:
            ...
            pending = [i for i, value in enumerate(existing) if value.get("quantity") is None]
            if (edit.action == "UPDATE" and pending
                    and not _quote(item.get("name", "").casefold(), edit.evidence.casefold())
                    and (len(pending) != 1 or index != pending[0])):
                raise OrderClarificationRequired("AMBIGUOUS_ORDER")
        ...
        if not item.get("quantity") and (not allow_partial or require_full):
    ...
    if (not allow_partial or require_full) and any(item.get("quantity") is None for item in result):
        raise OrderClarificationRequired("MISSING_QUANTITY")
...
def understand_order(request, message, existing, *, require_full=False, allow_partial=False):
    ...
    context = {"currentMessage": message.text, "guestLocale": request.guest.preferredLanguage,
               ...
               "allowPartialDraft": allow_partial and not require_full}
...
When allowPartialDraft is true, preserve a clearly identified product even without its quantity:
return RESOLVED with quantity/quantityEvidence null; do not guess one or discard other items.
If exactly one existing item lacks quantity, a short quantity-only reply such as 'Uno' or 'two'
updates THAT item using PATCH/UPDATE, its index and original quantity evidence. Keep its name
and restrictions. If several items lack quantities, require an identified target; do not guess.
Completing a missing quantity is not a full replacement. Keep all other existing items.
...
""" + ORDER_QUANTITY_POLICY + "\nContext:\n" + json.dumps(context, ensure_ascii=False)
    ...
    try:
        ...
        items = apply_order_extraction(extraction, message.text, existing, require_full, allow_partial=allow_partial)
