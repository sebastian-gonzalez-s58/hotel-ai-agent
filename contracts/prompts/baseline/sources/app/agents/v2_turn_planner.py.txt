...
def _room_service_capture_request(request, scope):
    """Keep Telware's in-progress capture even if scope calls the reply a new service."""
    if request.previousToolResults or scope.kind != "SERVICE_REQUEST" or scope.offeringCode != "ROOM_SERVICE":
        return request, scope
    latest = _latest_inbound_message(request)
    if latest is None:
        return request, scope
    separate = scope.separateRequest
    if not understanding_enabled() and _explicit_separate_order(latest.text):
        separate = True
        scope = scope.model_copy(update={"hasRequestDetails": False, "separateRequest": True})
    if separate:
        return request, scope
    state = _latest_capture_state(request)
    saved = state.get("roomServiceDraft")
    # A suspended draft still goes through the fresh-summary boundary before approval.
    if state.get("pendingOffering") != "ROOM_SERVICE" and isinstance(saved, dict):
        if not understanding_enabled() and scope.replyAction == "NONE":
            scope = scope.model_copy(update={"hasRequestDetails": True})
        return request, scope
...
def _preserve_room_service_draft(request, response):
    ...
    latest = _latest_inbound_message(request)
    if (isinstance(draft, dict) and not old.get("pendingOffering") and not new
            and not request.previousToolResults and latest is not None
            and _is_free_text_cancel(latest.text)):
        return response
...
def _room_service_draft_plan(
    ...
    if selection is not None:
        if selection[1] == "deliveryLocation":
            ...
            return None
        return _deterministic_turn_response(request, started_at, disposition="RESPONSE_READY",
            **_room_service_capture_output(request, offering, captured))
    if not understanding_enabled():
...
def _room_service_capture_output(request, offering, captured, unresolved_location=False):
    ...
    if pending:
        ...
        awaiting = False
    elif not captured.get("items"):
        field = offering.inputSchema.get("properties", {}).get("items", {})
        message = _capture_message(request, offering.offeringCode, "items", field,
                                   field.get("x-chatbotinn-capture", {}))
        message = message or _order_clarification_message(request)
        phase, awaiting = "CAPTURING_ITEMS", False
    else:
