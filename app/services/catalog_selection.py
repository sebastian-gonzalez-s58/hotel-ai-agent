...
class PendingCatalogSelection:
    ...
    def context(self):
        return {"offeringCode": self.offering.offeringCode, "fieldCode": self.field_code,
...
def pending_catalog_selection(request, state):
    ...
    for code, field in ordered_capture_fields(offering):
        if captured.get(code) not in (None, "", [], {}):
            continue
        ...
        return PendingCatalogSelection(offering, code, field, options) if options else None
    return None
